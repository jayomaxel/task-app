...
以下内容描述的是 Task Chain 与每日规划场景下的前端交互设计方向，当前代码库尚未完整实现这些界面与交互。

### 7.1 新增视图：Chain 视图

在现有 Today / All / Timeline / AI Assistant 基础上，规划新增 Chain 视图，作为任务链路的主操作界面。

#### 7.1.1 链路列表

- 显示所有链路卡片
- 每个卡片展示链路标题、进度条和当前阶段标签
- 进度条使用“已完成数 / 总任务数”的方式表达整体推进情况
- 点击链路卡片后进入链路详情页
...
- 子任务以纵向流程图方式展示
- 节点之间通过连线表示依赖关系
- 每个节点显示任务名称、状态徽标和预估耗时
- 点击节点后展开详情面板，展示目标、前置条件、风险说明和下一步建议
- 支持拖拽排序，并在交互完成后更新 `chain_order`
- 支持右键菜单操作，包括编辑、删除、标记状态、插入新任务、拆分和合并

#### 7.1.3 AI 调整输入框

链路详情页底部规划提供一个自然语言输入框，用户可以直接输入调整指令，例如：

- “把第三步删掉”
- “加一个测试环节”
- “这个步骤先跳过”

提交后调用 `/api/ai/chain-adjust`，并以动画形式更新流程图中的节点与连线变化。

### 7.2 每日规划界面

- 在 Today 视图顶部新增“今日空闲时间”编辑区
- 时间段通过横向时间轴可视化展示
- 用户可以通过拖拽方式调整时间段的起止时间
- 下方展示 AI 生成的当日排程结果，并允许用户手动微调
- 提供一键“重新排程”按钮，用于基于新空闲时间重新生成当日安排

### 7.3 AI 组合推荐卡片

在链路详情页的每个子任务节点旁边，规划支持展开“AI 工具推荐”卡片。
...
只有当用户已经配置 AI Profile 后，前端才显示这一能力入口。

## 第九部分：边界情况与异常场景（规划）

以下内容描述的是 Task Chain、AI 调整和每日排程场景下需要重点处理的边界情况与异常处理策略，当前代码库尚未完整实现这些规则。

| 场景 | 处理方式 |
| --- | --- |
| 循环依赖，例如 A 依赖 B、B 依赖 A | 后端在写入 `depends_on` 时执行环检测，可采用 DFS；一旦发现环路，返回 `400 Bad Request` |
| 删除唯一前置任务 | 下游任务的 `depends_on` 自动变为空数组，表示无依赖并转为可执行 |
| AI 返回的 `temp_id` 映射失败 | 忽略无法映射的依赖，记录 warning 日志，不阻塞整条链路创建 |
| AI 调整指令含糊不清 | AI 返回 `clarification_needed` 字段，前端提示用户补充信息 |
| 链路中所有任务都完成 | 将链路整体标记为 `completed`，并在链路列表中展示已完成徽标 |
| 用户修改了 AI 刚调整的内容 | 以用户修改为准，并将 `modified_by` 更新为 `user` |
| AI 调整时链路正在被用户编辑 | 后端使用行级锁控制并发；后到请求等待，或直接返回 `409 Conflict` |
| 空闲时间不足以安排所有任务 | AI 返回排程结果以及 `unscheduled` 列表，前端展示未安排原因 |
| 任务链路非常长，大于 20 个子任务 | 在 AI prompt 中只传递关键字段，降低 token 消耗和响应复杂度 |
| AI Provider 不可用 | 复用现有本地兜底逻辑，将链路拆分降级为普通子任务列表 |
